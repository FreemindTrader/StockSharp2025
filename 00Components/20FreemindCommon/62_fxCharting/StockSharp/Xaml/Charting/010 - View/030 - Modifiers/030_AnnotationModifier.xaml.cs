...
public partial class AnnotationModifier : fxAnnotationCreationModifier, IComponentConnector
{
    ...
    private void AddMenuItems(AnnotationBase annoBase, bool hasAnnotation)
    {
        ...
        EventHandler<EventArgs> dragEndedCommand = (s, e) =>
        {
            ...
            try
            {
                UltrachartAnnotationEditor editor = this.GetAnnotationEditor();
                if ( !editor.IsOpen )
                    return;
                editor.IsOpen = false;
                editor.IsOpen = true;
            ...
            finally
            {
                ...
                if ( this._baseToAnnotationPair.TryGetValue(annotationBase, out chartAnnotation) )
                    this.ChartArea.InvokeAnnotationModifiedEvent(chartAnnotation, this.GetAnnotationData(annotationBase));
            }
        };
        ...
        if ( hasAnnotation )
        {
            ...
            annoBase.Unselected +=  (s, e) => this.GetAnnotationEditor().IsOpen = false;
            annoBase.DragStarted += (s, e) => this._isUpdating = true;
    ...
    private void AddDependencyProperties(AnnotationBase b)
    {
        ...
        b.Selected += (s, e) =>
        {
            ...
            var a = CollectionHelper.TryGetValue(this._baseToAnnotationPair, myBase);
        ...
        {
            ChartAnnotation chartAnnotation;
            if ( !( _baseToAnnotationPair ).TryGetValue(b, out chartAnnotation) || this._isUpdating )
                return;

            this.ChartArea.InvokeAnnotationModifiedEvent(chartAnnotation, this.GetAnnotationData(b));
        }));
    }

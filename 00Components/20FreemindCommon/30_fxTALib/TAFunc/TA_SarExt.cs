...
namespace fx.TALib
{
    ...
    {
        public static RetCode SarExt( IHistoricBarsRepo bars, int startIdx, int endIdx, double[ ] outReal, out int outBegIdx,
            ...
            double optInAccelerationShort = 0.02, double optInAccelerationMaxShort = 0.2 )
        {
            ...
            if ( bars == null || outReal == null || optInOffsetOnReverse < 0.0 || optInAccelerationInitLong < 0.0 ||
            ...
            if ( optInStartValue.Equals( 0.0 ) )
            {
                ...
                RetCode retCode = MinusDM( bars, startIdx, startIdx, epTemp, out _, out _, 1);
            ...
            double newHigh = bars[todayIdx - 1].High;
            double newLow = bars[todayIdx - 1].Low;
            if ( optInStartValue.Equals( 0.0 ) )
            {
                if ( isLong )
                {
                    ep = bars[ todayIdx ].High;
                    sar = newLow;
                }
                else
                {
                    ep = bars[ todayIdx ].Low;
                    sar = newHigh;
                }
            }
            else if ( optInStartValue > 0.0 )
            ...
            newLow = bars[ todayIdx ].Low;
            newHigh = bars[ todayIdx ].High;

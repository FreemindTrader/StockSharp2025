...
// Type: StockSharp.Xaml.Charting.Model.DataSeries.SByteDataDistributionCalculator
...
namespace StockSharp.Xaml.Charting.Model.DataSeries
{
    internal class SByteDataDistributionCalculator : BaseDataDistributionCalculator<sbyte>
    {
        ...
        {
            UpdateDataDistributionFlagsWhenAppendedXValue( xValues, newXValue, ( ( ICollection<sbyte> ) xValues ).Count - 1, acceptsUnsortedData );
        }
        ...
        {
            sbyte[] newXValues1 = newXValues as sbyte[];
            if ( newXValues1 != null )
            {
                UpdateDataDistributionFlagsWhenAppendedDataArray( ( IList<sbyte> ) xValues, countBeforeAppending, newXValues1, newXValues1.Length, acceptsUnsortedData );
            }
            else
            {
                IList<sbyte> list = newXValues as IList<sbyte>;
                if ( list != null )
                {
                    int count = list.Count;
                    sbyte[] uncheckedList = list.ToUncheckedList<sbyte>();
                    UpdateDataDistributionFlagsWhenAppendedDataArray( ( IList<sbyte> ) xValues, countBeforeAppending, uncheckedList, count, acceptsUnsortedData );
                }
                else
                {
                    IEnumerable<sbyte> sbytes = newXValues;
                    int lastIndexAfterAppending = countBeforeAppending;
                    foreach ( sbyte newXValue in sbytes )
                    {
        ...
        public override void OnInsertXValue( ISeriesColumn<sbyte> xValues2, int indexWhereInserted, sbyte newXValue, bool acceptsUnsortedData )
        {
            if ( !DataIsSortedAscending && !DataIsEvenlySpaced )
                return;
            IList<sbyte> sbyteList = (IList<sbyte>) xValues2;
            int count = sbyteList.Count;
            if ( indexWhereInserted == 0 )
            {
                if ( count <= 1 )
                    return;
                sbyte xValue = sbyteList[1];
                double num1 = SByteDataDistributionCalculator.TxToDouble(xValue) - SByteDataDistributionCalculator.TxToDouble(newXValue);
                _firstXDiff = num1;
                ...
                else
                {
                    if ( !DataIsEvenlySpaced || !_firstXDiffIsValid )
                        return;
                    double num2 = num1 - (SByteDataDistributionCalculator.TxToDouble(sbyteList[2]) - SByteDataDistributionCalculator.TxToDouble(xValue));
                    if ( num2 < 0.0 )
                        num2 = -num2;
            ...
            else
            {
                ...
                if ( !DataIsSortedAscending )
                    return;
                if ( indexWhereInserted > 0 && ( int ) sbyteList[ indexWhereInserted - 1 ] > ( int ) newXValue )
                {
                    DataIsSortedAscending = false;
                    if ( !DataIsSortedAscending && !acceptsUnsortedData )
                        throw new InvalidOperationException( "Data has been Inserted to a DataSeries which is unsorted in the X-Direction. Unsorted data can have severe performance implications in Ultrachart.\r\nFor maximum performance, please double-check that you are only inserting sorted data to Ultrachart. Alternatively, to disable this warning and allow unsorted data, please set DataSeries.AcceptsUnsortedData = true. For more info see Performance Tips and Tricks at http://support.ultrachart.com/index.php?/Knowledgebase/Article/View/17227/36/performance-tips-and-tricks" );
                }
                if ( indexWhereInserted >= count - 1 || ( int ) sbyteList[ indexWhereInserted + 1 ] >= ( int ) newXValue )
                    return;
                DataIsSortedAscending = false;
                if ( !DataIsSortedAscending && !acceptsUnsortedData )
                    throw new InvalidOperationException( "Data has been Inserted to a DataSeries which is unsorted in the X-Direction. Unsorted data can have severe performance implications in Ultrachart.\r\nFor maximum performance, please double-check that you are only inserting sorted data to Ultrachart. Alternatively, to disable this warning and allow unsorted data, please set DataSeries.AcceptsUnsortedData = true. For more info see Performance Tips and Tricks at http://support.ultrachart.com/index.php?/Knowledgebase/Article/View/17227/36/performance-tips-and-tricks" );
        ...
        public override void OnInsertXValues( ISeriesColumn<sbyte> xValues2, int indexWhereInserted, int insertedCount, IEnumerable<sbyte> newXValues, bool acceptsUnsortedData )
        {
            ...
            else if ( indexWhereInserted == 0 )
            {
                if ( count <= 2 )
                    return;
                double num1 = SByteDataDistributionCalculator.TxToDouble(sbyteList[1]) - SByteDataDistributionCalculator.TxToDouble(sbyteList[0]);
                _firstXDiff = num1;
                _firstXDiffIsValid = true;
                ...
                {
                    double num2 = SByteDataDistributionCalculator.TxToDouble(sbyteList[index]);
        ...
        private void UpdateDataDistributionFlagsWhenAppendedXValue( ISeriesColumn<sbyte> xValues, sbyte newXValue, int lastIndexAfterAppending, bool acceptsUnsortedData )
        {
            if ( !DataIsSortedAscending && !DataIsEvenlySpaced )
                return;
            if ( lastIndexAfterAppending > 0 )
            {
                double num1 = SByteDataDistributionCalculator.TxToDouble(newXValue) - _lastXValue;
                if ( num1 < 0.0 )
                {
                    DataIsSortedAscending = false;
                    DataIsEvenlySpaced = false;
            ...
            _lastXValue = SByteDataDistributionCalculator.TxToDouble( newXValue );
        }

        private void UpdateDataDistributionFlagsWhenAppendedDataArray( IList<sbyte> xValues, int countBeforeAppending, sbyte[ ] newXValues, int newXValuesLength, bool acceptsUnsortedData )
        {
            if ( !DataIsSortedAscending && !DataIsEvenlySpaced )
                return;
            ...
            {
                double num2 = SByteDataDistributionCalculator.TxToDouble(newXValues[0]) - _lastXValue;
                if ( DataIsSortedAscending && _lastXValue > SByteDataDistributionCalculator.TxToDouble( newXValues[ 0 ] ) )
                {
                    DataIsSortedAscending = false;
                    if ( !DataIsSortedAscending && !acceptsUnsortedData )
                        throw new InvalidOperationException( "Data has been Appended to a DataSeries which is unsorted in the X-Direction. Unsorted data can have severe performance implications in Ultrachart.\r\nFor maximum performance, please double-check that you are only inserting sorted data to Ultrachart. Alternatively, to disable this warning and allow unsorted data, please set DataSeries.AcceptsUnsortedData = true. For more info see Performance Tips and Tricks at http://support.ultrachart.com/index.php?/Knowledgebase/Article/View/17227/36/performance-tips-and-tricks" );
            ...
            if ( !false )
            {
                double num2 = SByteDataDistributionCalculator.TxToDouble(newXValues[0]);
                double num3 = _firstXDiff;
                bool flag = _firstXDiffIsValid;
                for ( int index = 1 ; index < num1 ; ++index )
                {
                    double num4 = SByteDataDistributionCalculator.TxToDouble(newXValues[index]);
            ...
            _lastXValue = SByteDataDistributionCalculator.TxToDouble( newXValues[ num1 - 1 ] );
            if ( _firstXDiffIsValid || num1 <= 1 )
                return;
            _firstXDiff = _lastXValue - SByteDataDistributionCalculator.TxToDouble( newXValues[ num1 - 2 ] );
            _firstXDiffIsValid = true;
            _minVisibleXDiffError = _firstXDiff * 0.000125;
        }

...
// Type: StockSharp.Xaml.Charting.Model.DataSeries.ListDoubleDataDistributionCalculator
...
namespace StockSharp.Xaml.Charting.Model.DataSeries
{
    internal sealed class ListDoubleDataDistributionCalculator : BaseDataDistributionCalculator<double>
    {
        ...
        {
            OnAppend( ( IList<double> ) xValues, 1 );
        ...
        {
            OnAppend( ( IList<double> ) xValues, ( ( ISeriesColumn ) xValues ).Count - countBeforeAppending );
            if ( !acceptsUnsortedData && !IsSortedAscending )
            {
                throw new InvalidOperationException( "Data has been Appended to a DataSeries which is unsorted in the X-Direction. Unsorted data can have severe performance implications in Ultrachart.\r\nFor maximum performance, please double-check that you are only inserting sorted data to Ultrachart. Alternatively, to disable this warning and allow unsorted data, please set DataSeries.AcceptsUnsortedData = true. For more info see Performance Tips and Tricks at http://support.ultrachart.com/index.php?/Knowledgebase/Article/View/17227/36/performance-tips-and-tricks" );
            }
        ...
        {
            OnInsert( ( IList<double> ) xValues, indexWhereInserted, 1 );
            if ( !acceptsUnsortedData && !IsSortedAscending )
            {
                throw new InvalidOperationException( "Data has been Appended to a DataSeries which is unsorted in the X-Direction. Unsorted data can have severe performance implications in Ultrachart.\r\nFor maximum performance, please double-check that you are only inserting sorted data to Ultrachart. Alternatively, to disable this warning and allow unsorted data, please set DataSeries.AcceptsUnsortedData = true. For more info see Performance Tips and Tricks at http://support.ultrachart.com/index.php?/Knowledgebase/Article/View/17227/36/performance-tips-and-tricks" );
            }
        ...
        public override void OnInsertXValues( ISeriesColumn<double> xValues, int indexWhereInserted, int insertedCount, IEnumerable<double> newXValues, bool acceptsUnsortedData )
        {
            OnInsert( ( IList<double> ) xValues, indexWhereInserted, newXValues.Count<double>() );
            if ( !acceptsUnsortedData && !IsSortedAscending )
            {
                {
                    {
        ...
        public void OnAppend( IList<double> valuesAfterAppend, int appendedCount )
        {
            ...
            IsSortedAscending = ( appendedCount == 1 || ArrayOperations.IsSortedAscending<double>( valuesAfterAppend, startIndex, appendedCount ) ) & ( count <= 1 || startIndex <= 0 || valuesAfterAppend[ startIndex ] >= valuesAfterAppend[ startIndex - 1 ] );
            if ( !IsEvenlySpaced )
            {
                {
                    {
            ...
            if ( ( appendedCount == 1 ? 1 : ( ArrayOperations.IsEvenlySpaced<double>( valuesAfterAppend, startIndex, appendedCount, epsilon, out spacing ) ? 1 : 0 ) ) == 0 )
            {
                {
                    {
                        {
            ...
            {
                double num1 = ListDoubleDataDistributionCalculator.TxToDouble(valuesAfterAppend[startIndex - 1]);
                double num2 = ListDoubleDataDistributionCalculator.TxToDouble(valuesAfterAppend[startIndex]) - num1;
        ...
        public void OnInsert( IList<double> valuesAfterInsert, int insertedIndex, int insertedCount )
        {
            ...
            bool flag = insertedCount == 1 || ArrayOperations.IsSortedAscending<double>(valuesAfterInsert, insertedIndex, insertedCount);
            ...
            double spacing = 1.0;
            double epsilon = _currentSpacing * 0.000125;
            if ( ( insertedCount == 1 ? 1 : ( ArrayOperations.IsEvenlySpaced<double>( valuesAfterInsert, insertedIndex, insertedCount, epsilon, out spacing ) ? 1 : 0 ) ) == 0 )
            {
                IsEvenlySpaced = false;
            }
            ...
            else
            {
                if ( insertedIndex > 0 )
                {
                    double num1 = ListDoubleDataDistributionCalculator.TxToDouble(valuesAfterInsert[insertedIndex - 1]);
                    double num2 = ListDoubleDataDistributionCalculator.TxToDouble(valuesAfterInsert[insertedIndex]) - num1;
                ...
                {
                    double num1 = ListDoubleDataDistributionCalculator.TxToDouble(valuesAfterInsert[index]);
                    double num2 = ListDoubleDataDistributionCalculator.TxToDouble(valuesAfterInsert[index + 1]) - num1;
                    if ( valuesAfterInsert.Count > 2 )
                    {
                        if ( Math.Abs( num2 - _currentSpacing ) > epsilon )
                        {

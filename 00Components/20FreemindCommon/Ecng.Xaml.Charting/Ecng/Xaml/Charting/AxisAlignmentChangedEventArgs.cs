// Decompiled with JetBrains decompiler
// Type: StockSharp.Xaml.Charting.AxisAlignmentChangedEventArgs
// Assembly: StockSharp.Xaml.Charting, Version=1.0.0.0, Culture=neutral, PublicKeyToken=b10e79ed0227b515
// MVID: C2F11401-C1E6-47FC-9255-FC66EA027789
// Assembly location: A:\10 - StockSharp\Hydra\StockSharp.Xaml.Charting.dll

using System;
using StockSharp.Xaml.Charting.Visuals.Axes;

namespace StockSharp.Xaml.Charting
{
    public class AxisAlignmentChangedEventArgs : EventArgs
    {
        public AxisAlignment OldAlignment
        {
            get; private set;
        }

        public AxisAlignment NewAlignment
        {
            get; private set;
        }

        public string AxisId
        {
            get; private set;
        }

        public AxisAlignmentChangedEventArgs( string axisId, AxisAlignment oldValue, AxisAlignment newValue )
        {
            this.OldAlignment = oldValue;
            this.NewAlignment = newValue;
            this.AxisId = axisId;
        }
    }
}

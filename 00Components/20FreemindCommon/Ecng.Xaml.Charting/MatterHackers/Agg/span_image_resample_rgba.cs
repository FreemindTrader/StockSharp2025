...
// Type: MatterHackers.Agg.span_image_resample_rgba
...
{
    internal class span_image_resample_rgba : span_image_resample
    {
          ...
          : base( src, inter, filter )
        {
            if ( src.SourceImage.GetBlender().NumPixelBits != 32 )
                throw new FormatException( "You have to use a rgba blender with span_image_resample_rgba" );
        }
        ...
        public override void generate( RGBA_Bytes[ ] span, int spanIndex, int x, int y, int len )
        {
            ...
            do
            {
                ...
                numArray1[ 0 ] = numArray1[ 1 ] = numArray1[ 2 ] = numArray1[ 3 ] = 8192;
                int y2 = y >> 8;
                ...
                while ( true )
                {
                    ...
                    while ( true )
                    {
                        ...
                        numArray1[ 3 ] += ( int ) numArray4[ index2 + 3 ] * num10;
                        num7 += num10;
                        index3 += num3;
                        if ( index3 < num2 )
                ...
                span[ spanIndex ].alpha = ( byte ) numArray1[ 3 ];
                ++spanIndex;

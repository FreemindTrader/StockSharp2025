...
// Type: MatterHackers.Agg.Image.blenderGrayFromRed
...
namespace MatterHackers.Agg.Image
{
    internal class blenderGrayFromRed : IBlenderByte
    {
        ...
        public blenderGrayFromRed( int bytesBetweenPixelsInclusive )
        {
            ...
            if ( blenderGrayFromRed.m_Saturate9BitToByte[ 2 ] != 0 )
                return;
            for ( int val1 = 0 ; val1 < blenderGrayFromRed.m_Saturate9BitToByte.Length ; ++val1 )
                blenderGrayFromRed.m_Saturate9BitToByte[ val1 ] = Math.Min( val1, ( int ) byte.MaxValue );
        }
        ...
        {
            byte num = (byte) (((int) sourceColor.red - (int) pDestBuffer[bufferOffset]) * (int) sourceColor.alpha + ((int) pDestBuffer[bufferOffset] << 8) >> 8);

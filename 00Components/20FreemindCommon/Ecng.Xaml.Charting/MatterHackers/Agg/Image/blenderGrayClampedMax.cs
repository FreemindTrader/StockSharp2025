...
// Type: MatterHackers.Agg.Image.blenderGrayClampedMax
...
namespace MatterHackers.Agg.Image
{
    internal class blenderGrayClampedMax : IBlenderByte
    {
        ...
        public blenderGrayClampedMax( int bytesBetweenPixelsInclusive )
        {
            ...
            if ( blenderGrayClampedMax.m_Saturate9BitToByte[ 2 ] != 0 )
                return;
            for ( int val1 = 0 ; val1 < blenderGrayClampedMax.m_Saturate9BitToByte.Length ; ++val1 )
                blenderGrayClampedMax.m_Saturate9BitToByte[ val1 ] = Math.Min( val1, ( int ) byte.MaxValue );
        }
        ...
        public void CopyPixels( byte[ ] pDestBuffer, int bufferOffset, RGBA_Bytes sourceColor, int count )
        {
            do
            {
                byte num = Math.Min(Math.Max(sourceColor.red, Math.Max(sourceColor.green, sourceColor.blue)), byte.MaxValue);
                pDestBuffer[ bufferOffset ] = num;
        ...
        {
            byte num = (byte) (((int) Math.Min(Math.Max(sourceColor.red, Math.Max(sourceColor.green, sourceColor.blue)), byte.MaxValue) - (int) pDestBuffer[bufferOffset]) * (int) sourceColor.alpha + ((int) pDestBuffer[bufferOffset] << 8) >> 8);
            pDestBuffer[ bufferOffset ] = num;
        }

...
#pragma warning restore 420
    ...
    public class TonyBatchEventProcessor<T, TDataProvider, TSequenceBarrier, TEventHandler, TBatchStartAware> : IBatchEventProcessor<T>
        where T : class
        where TDataProvider : IDataProvider<T>
        where TSequenceBarrier : ISequenceBarrier
        where TEventHandler : IEventHandler<T>
        where TBatchStartAware : IBatchStartAware
    {
        ...
        /// <summary>
        /// Construct a TonyBatchEventProcessor that will automatically track the progress by updating its sequence when
        /// the <see cref="IEventHandler{T}.OnEvent"/> method returns.
        ///
        /// Consider using <see cref="BatchEventProcessorFactory"/> to create your <see cref="IEventProcessor"/>.
        /// </summary>
        ...
        /// <param name="eventHandler">eventHandler is the delegate to which events are dispatched.</param>
        /// <param name="batchStartAware"></param>
        public TonyBatchEventProcessor( TDataProvider dataProvider, TSequenceBarrier sequenceBarrier, TEventHandler eventHandler, TBatchStartAware batchStartAware )
        {
            _dataProvider = dataProvider;
            _sequenceBarrier = sequenceBarrier;
            _eventHandler = eventHandler;
        ...
        private void ProcessEvents()
        {
            ...
            while ( true )
            {
                try
                {
                    ...
                    // However, this test has a negative impact on performance even for event handlers that are not IBatchStartAware.
                    // This is unfortunate because this test should be removed by the JIT when OnBatchStart is a noop.
                    // => The test is currently implemented on struct proxies. See TonyBatchEventProcessor<T>.BatchStartAware and StructProxy.
                    // For some reason this also improves TonyBatchEventProcessor performance for IBatchStartAware event handlers.

                    _batchStartAware.OnBatchStart( availableSequence - nextSequence + 1 );

                    while ( nextSequence <= availableSequence )
                    {
                        while ( _running == RunningStates.Halted )
                        {
                            Thread.Sleep( 500 );
                        }

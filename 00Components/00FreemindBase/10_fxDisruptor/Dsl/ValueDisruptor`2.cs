...
namespace Disruptor.Dsl
{
        ...
        where TRingBuffer : IValueRingBuffer<T>
    {
        ...
        public ValueEventHandlerGroup<T> TonyHandleEventsWith( params IValueEventHandler<T>[ ] handlers )
        {
            return TonyCreateEventProcessors( new ISequence[ 0 ], handlers );
        }
        ...
        private ValueEventHandlerGroup<T> TonyCreateEventProcessors( ISequence[ ] barrierSequences, IValueEventHandler<T>[ ] eventHandlers )
        {
            CheckNotStarted();
            ...
            for ( int i = 0; i < eventHandlers.Length; i++ )
            {
                ...
                var batchEventProcessor = BatchEventProcessorFactory.CreateModified(_ringBuffer, barrier, eventHandler);

...
public static class StrategyExtensions
{
    ...
    public static SecurityId ToSecurityId(this InstrumentInfo instrument)
    {
        ...
        securityId.SecurityCode = instrument.Code.ToUpperInvariant();
        securityId.BoardCode = instrument.Board.ToUpperInvariant();
        return securityId;
    }

    public static InstrumentInfo ToInstrumentInfo(this string stringId)
    {
        SecurityId securityId = Extensions.ToSecurityId(stringId, (SecurityIdGenerator)null);
    ...
    {
        return StringHelper.ThrowIfEmpty(fileNameNoExt, nameof(fileNameNoExt)) + $".{type}".ToLowerInvariant();
    }

    private static StrategyReportTypes GetReportType(string fileName)
    {
        return Converter.To<StrategyReportTypes>((object)Path.GetExtension(fileName).Substring(1));
    }
      ...
      CancellationToken cancellationToken)
    {
        ...
        StrategyReportTypes strategyReportTypes = file != null ? StrategyExtensions.GetReportType(file.Name) : throw new ArgumentNullException(nameof(file));
        Stream stream = Converter.To<Stream>((object)file.Body);
        if (strategyReportTypes != StrategyReportTypes.Zip)
        {
            if (strategyReportTypes == StrategyReportTypes.Json)
                return await ((Serializer<TReport>)serializer).DeserializeAsync(stream, cancellationToken);
            throw new ArgumentOutOfRangeException("type", strategyReportTypes.ToString());
        }
        using (IEnumerator<(string, Stream)> enumerator = CompressionHelper.Unzip(stream, true, (Func<string, bool>)null).GetEnumerator())
        {
    ...
    private static async ValueTask<StockSharp.Web.DomainModel.File> GenerateFile<TReport>(
      TReport report,
      JsonSerializer<TReport> serializer,
      StrategyReportTypes type,
      ...
      CancellationToken cancellationToken)
    {
        ...
        string file1 = await StrategyExtensions.GenerateFile<TReport>(report, serializer, type, (Stream)body, fileNameNoExt, cancellationToken);
        StockSharp.Web.DomainModel.File file2 = new StockSharp.Web.DomainModel.File()
        {
            Name = file1,
            Body = Converter.To<byte[]>((object)body)
        };
    ...
    {
        StrategyPositionChange[] changes = TypeHelper.CheckOnNull<StrategyPosition>(position, nameof(position)).Changes;
        if (changes == null)
            return new Decimal?();
    ...
    {
        return respose.EntityId.HasValue || !StringHelper.IsEmpty(respose.UserId);
    }
}

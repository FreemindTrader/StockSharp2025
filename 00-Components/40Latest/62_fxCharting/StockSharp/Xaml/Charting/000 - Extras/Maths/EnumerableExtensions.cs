...
public static class EnumerableExtensions
{
    ...
    public static int FindIndex(this IList list, bool isSorted, IComparable value, SearchMode searchMode)
    {
        ...
        throw new NotImplementedException(
            string.Format(
                ...
                (object) SearchMode.Exact,
                (object) searchMode));
    }

    private static int FindIndexInSortedData(
        this IList list,
        IComparable value,
        IComparer<IComparable> comparer,
    ...
    internal static int FindIndexInSortedData<TX>(
        this TX[ ] array,
        int arrayLength,
        TX value,
        SearchMode searchMode,
    ...
    private static int GetNearestMiddleIndex<TX>(
        TX[ ] array,
        int arrayLength,
        int lower,
        int upper,
    ...
    internal static IEnumerable<IEnumerable<Tuple<Point, Point>>> SplitMultilineByGaps(
        this IEnumerable<Tuple<Point, Point>> points)
    {
    ...
    private static IEnumerable<Tuple<Point, Point>> EnumerateUntilGap(IEnumerator<Tuple<Point, Point>> iterator)
    {
        if(!double.IsNaN(iterator.Current.Item1.X) && !double.IsNaN(iterator.Current.Item1.Y))
        {
            ...
            while(iterator.MoveNext() &&
                (!double.IsNaN(iterator.Current.Item1.X) && !double.IsNaN(iterator.Current.Item1.Y)))
            {
                yield return iterator.Current;
            }
        }

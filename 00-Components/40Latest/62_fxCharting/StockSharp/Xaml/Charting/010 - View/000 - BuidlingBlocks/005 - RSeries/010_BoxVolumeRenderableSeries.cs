...
/// <summary>
/// The following in general is a point and figure chart drawing series.
/// </summary>
public sealed class BoxVolumeRenderableSeries : TimeframeSegmentRenderableSeries
{
    public static readonly DependencyProperty Timeframe2Property           = DependencyProperty.Register(nameof(Timeframe2),           typeof(TimeSpan?), typeof(BoxVolumeRenderableSeries), new PropertyMetadata(TimeSpan.FromMinutes(5.0), new PropertyChangedCallback(OnInvalidateParentSurface), new CoerceValueCallback(CoerceHigherTimeframe)));

    public static readonly DependencyProperty Timeframe3Property           = DependencyProperty.Register(nameof(Timeframe3),           typeof(TimeSpan?), typeof(BoxVolumeRenderableSeries), new PropertyMetadata(TimeSpan.FromMinutes(15.0), new PropertyChangedCallback(OnInvalidateParentSurface), new CoerceValueCallback(CoerceHigherTimeframe)));

    public static readonly DependencyProperty Timeframe2ColorProperty      = DependencyProperty.Register(nameof(Timeframe2Color),      typeof(Color), typeof(BoxVolumeRenderableSeries),     new PropertyMetadata(Color.FromRgb((byte)36, (byte)36, (byte)36), new PropertyChangedCallback(OnInvalidateParentSurface)));

    public static readonly DependencyProperty Timeframe2FrameColorProperty = DependencyProperty.Register(nameof(Timeframe2FrameColor), typeof(Color), typeof(BoxVolumeRenderableSeries),     new PropertyMetadata(Color.FromRgb(byte.MaxValue, (byte)102, (byte)0), new PropertyChangedCallback(OnInvalidateParentSurface)));

    public static readonly DependencyProperty Timeframe3ColorProperty      = DependencyProperty.Register(nameof(Timeframe3Color),      typeof(Color), typeof(BoxVolumeRenderableSeries),     new PropertyMetadata(Color.FromRgb((byte)0, (byte)55, (byte)24), new PropertyChangedCallback(OnInvalidateParentSurface)));

    public static readonly DependencyProperty CellFontColorProperty        = DependencyProperty.Register(nameof(CellFontColor),        typeof(Color), typeof(BoxVolumeRenderableSeries),     new PropertyMetadata(Color.FromRgb((byte)90, (byte)90, (byte)90), new PropertyChangedCallback(OnInvalidateParentSurface)));

    public static readonly DependencyProperty HighVolColorProperty         = DependencyProperty.Register(nameof(HighVolColor),         typeof(Color), typeof(BoxVolumeRenderableSeries),     new PropertyMetadata(Colors.LawnGreen, new PropertyChangedCallback(OnInvalidateParentSurface)));

    public static readonly DependencyProperty HighVolBackgroundProperty    = DependencyProperty.Register(nameof(HighVolBackground),    typeof(Color), typeof(BoxVolumeRenderableSeries),     new PropertyMetadata(Colors.LightBlue, new PropertyChangedCallback(OnInvalidateParentSurface)));
    ...
    public TimeSpan? Timeframe2
    {
        get
        {
            return ( TimeSpan? ) GetValue( Timeframe2Property );
        }
        set
        {
            SetValue( Timeframe2Property, value );
        }
    }
    ...
    public Color Timeframe2Color
    {
        get
        {
            return ( Color ) GetValue( Timeframe2ColorProperty );
        }
        set
        {
            SetValue( Timeframe2ColorProperty, value );
        }
    }
    ...
    public Color Timeframe2FrameColor
    {
        get
        {
            return ( Color ) GetValue( Timeframe2FrameColorProperty );
        }
        set
        {
            SetValue( Timeframe2FrameColorProperty, value );
        }
    }
    ...
    public TimeSpan? Timeframe3
    {
        get
        {
            return ( TimeSpan? ) GetValue( Timeframe3Property );
        }
        set
        {
            SetValue( Timeframe3Property, value );
        }
    }
    ...
    public Color Timeframe3Color
    {
        get
        {
            return ( Color ) GetValue( Timeframe3ColorProperty );
        }
        set
        {
            SetValue( Timeframe3ColorProperty, value );
        }
    }
    ...
    public Color CellFontColor
    {
        get
        {
            return ( Color ) GetValue( CellFontColorProperty );
        }
        set
        {
            SetValue( CellFontColorProperty, value );
        }
    }
    ...
    public Color HighVolColor
    {
        get
        {
            return ( Color ) GetValue( HighVolColorProperty );
        }
        set
        {
            SetValue( HighVolColorProperty, value );
        }
    }
    ...
    public Color HighVolBackground
    {
        get
        {
            return ( Color ) GetValue( HighVolBackgroundProperty );
        }
        set
        {
            SetValue( HighVolBackgroundProperty, value );
        }
    }
    ...
    {
        var box    = (BoxVolumeRenderableSeries)d;
        var period = (TimeSpan?)newVal;
    ...
    {
        TimeSpan? tf = Timeframe;
        if ( !tf.HasValue )
            return range;

        var tf2Ticks   = Timeframe2.HasValue ? Timeframe2.GetValueOrDefault().Ticks : 0L;
        var tf3Ticks   = Timeframe3.HasValue ? Timeframe3.GetValueOrDefault().Ticks : 0L;
        var maxTicks   = Math.Max(tf2Ticks, tf3Ticks);
        var multiplier = (int)( maxTicks / tf.Value.Ticks );
    ...
    protected override void OnDrawImpl( IRenderContext2D rc, IRenderPassData data )
    {
        ...
        var cellFontColor     = CellFontColor;
        var highVolColor      = HighVolColor;
        var highVolBackground = HighVolBackground;
        ...
        {
            throw new InvalidOperationException( string.Format( "invalid timeframes. tf1={0}, tf2={1}, tf3={2}", Timeframe, Timeframe2, Timeframe3 ) );
        }

        SciChartDebugLogger.Instance.WriteLine( "BoxVolume: started render {0} segments. Indexes: {1}-{2}, VisibleRange: {3}-{4}", segments.Length, segments[0].Segment.X, segments[segments.Length - 1].Segment.X, visibleRange.Min, visibleRange.Max );
        ...
        using ( PenManager pm = new PenManager( rc, false, ( float ) StrokeThickness, Opacity ) )
        {
            var tf3Pen   = pm.GetPen(Timeframe3Color);
            var tf2Pen   = pm.GetPen(Timeframe2Color);
            ...
            var tf2fPen  = pm.GetPen(Timeframe2FrameColor);
            var helper   = SeriesDrawingHelpersFactory.GetSeriesDrawingHelper(rc, CurrentRenderPassData);
            ...
            var upColorPen       = pm.GetPen(UpColor, new float?((float)( StrokeThickness + 2 )));
            var downColorPen     = pm.GetPen(DownColor, new float?((float)( StrokeThickness + 2 )));
            ...
            if ( xUnit >= 2.0 && yUnit >= 2.0 )
            {
                for ( int index = 0; index < length; ++index )
                {
                    ...
                    var hasValidFont     = _fontCalculator.HasFont(size, myVolume.IncreaseEffectiveScale());
                    ...
                    foreach ( KeyValuePair<double, CandlePriceLevel> keyValuePair in priceLevels.Where( p => p.Key > minDrawPrice && p.Key < maxDrawPrice ) )
                    {
                        ...
                        if ( totalVolume > 0M )
                        {
                            ...
                            if ( hasValidFont )
                            {
                                ...
                                (float, FontWeight, bool) tuple = _fontCalculator.GetFont(size, totalVolume.IncreaseEffectiveScale(), 0.0f);

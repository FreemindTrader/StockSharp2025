...
{
    private static readonly List<int> QuarterMonths = new List<int>((IEnumerable<int>) new int[5] { 1, 4, 7, 10, 13 });

    private static readonly List<int> HalfMonths    = new List<int>((IEnumerable<int>) new int[3] { 1, 7, 13 });

    private static readonly List<int> BiMonths      = new List<int>((IEnumerable<int>) new int[7] { 1, 3, 5, 7, 9, 11, 13 });
    ...
    {
        if ( dateSpan.Ticks % OneYear.Ticks == 0L &&
            current.Day == 1 &&
            current.Month == 1 ||
            dateSpan.Ticks % TimeSpanExtensions.FromMonths( 1 ).Ticks == 0L &&
            current.Day == 1 )
        {
            return true;
        }
    ...
    internal static DateTime RoundUp( DateTime current, TimeSpan dateSpan )
    {
        ...
        long num1 = (long) (int) (dateSpan.Ticks / OneMonth.Ticks);

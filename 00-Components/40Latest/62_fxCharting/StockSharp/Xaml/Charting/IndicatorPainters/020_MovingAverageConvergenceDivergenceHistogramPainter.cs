  ...
  BaseChartIndicatorPainter<MovingAverageConvergenceDivergenceHistogram>
{
    ...
    public MovingAverageConvergenceDivergenceHistogramPainter()
    {
        ...
        _macd = (IChartLineElement)new ChartLineElement()
        {
            Color = indicatorColorProvider.GetNextColor()
        };
        _signal = (IChartLineElement)new ChartLineElement()
        {
            Color = indicatorColorProvider.GetNextColor()
        };
        _histogram = (IChartLineElement)new ChartLineElement()
        {
            Style = DrawStyles.Histogram
        };
        AddChildElement((IChartElement)Macd);
        AddChildElement((IChartElement)Signal);
        AddChildElement((IChartElement)Histogram);
    }
    ...
    public IChartLineElement Macd => _macd;
    ...
    public IChartLineElement Signal => _signal;
    ...
    public IChartLineElement Histogram => _histogram;

    protected override bool OnDraw(
      MovingAverageConvergenceDivergenceHistogram indicator,
      IDictionary<IIndicator, IList<ChartDrawData.IndicatorData>> data)
    {
        return ( DrawValues(data[(IIndicator)indicator.Macd], (IChartElement)Macd)
             | ( DrawValues(data[(IIndicator)indicator.SignalMa], (IChartElement)Signal) )
             | ( DrawValues(data[(IIndicator)indicator.Macd], data[(IIndicator)indicator.SignalMa], (IChartElement)Histogram, (d1, d2) => d1 - d2) ) );
    }

    public override void Load(SettingsStorage storage)
    {
        base.Load(storage);
        PersistableHelper.Load((IPersistable)Macd, storage, "Macd");
        PersistableHelper.Load((IPersistable)Signal, storage, "Signal");
        PersistableHelper.Load((IPersistable)Histogram, storage, "Histogram");
    }

    public override void Save(SettingsStorage storage)
    {
        base.Save(storage);
        storage.SetValue<SettingsStorage>("Macd", PersistableHelper.Save((IPersistable)Macd));
        storage.SetValue<SettingsStorage>("Signal", PersistableHelper.Save((IPersistable)Signal));
        storage.SetValue<SettingsStorage>("Histogram", PersistableHelper.Save((IPersistable)Histogram));
    }
}

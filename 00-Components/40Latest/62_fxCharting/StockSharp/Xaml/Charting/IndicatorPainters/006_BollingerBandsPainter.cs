...
public class BollingerBandsPainter : BaseChartIndicatorPainter<BollingerBands>
{
    ...
    public BollingerBandsPainter()
    {
        ...
        AddChildElement( ( IChartElement ) ( _bollingBand = ( IChartBandElement ) chartBandElement ) );
        AddChildElement( ( IChartElement ) ( _bollingMean = ( IChartLineElement ) chartLineElement ) );
    ...
    public IChartBandElement Band => _bollingBand;
    ...
    public IChartLineElement MovingAverage => _bollingMean;

    protected override bool OnDraw(
      BollingerBands indicator,
      IDictionary<IIndicator, IList<ChartDrawData.IndicatorData>> data )
    {
        return ( 0 | ( DrawValues( data[ ( IIndicator ) indicator.UpBand ], data[ ( IIndicator ) indicator.LowBand ], ( IChartElement ) Band ) ? 1 : 0 ) | ( DrawValues( data[ ( IIndicator ) indicator.MovingAverage ], ( IChartElement ) MovingAverage ) ? 1 : 0 ) ) != 0;
    }

    public override void Load( SettingsStorage storage )
    {
        base.Load( storage );
        PersistableHelper.Load( ( IPersistable ) Band, storage, "Band" );
        PersistableHelper.Load( ( IPersistable ) MovingAverage, storage, "MovingAverage" );
    }

    public override void Save( SettingsStorage storage )
    {
        base.Save( storage );
        storage.SetValue<SettingsStorage>( "Band", PersistableHelper.Save( ( IPersistable ) Band ) );
        storage.SetValue<SettingsStorage>( "MovingAverage", PersistableHelper.Save( ( IPersistable ) MovingAverage ) );
    }
}

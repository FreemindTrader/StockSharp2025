...
public class StochasticOscillatorPainter : BaseChartIndicatorPainter<StochasticOscillator>
{
    ...
    public StochasticOscillatorPainter()
    {
        ...
        _lineK = (IChartLineElement)new ChartLineElement()
        {
            Color = indicatorColorProvider.GetNextColor()
        };
        _lineD = (IChartLineElement)new ChartLineElement()
        {
            Color = indicatorColorProvider.GetNextColor()
        };
        AddChildElement((IChartElement)K);
        AddChildElement((IChartElement)D);
    }
    ...
    public IChartLineElement K => _lineK;
    ...
    {
        base.Load(storage);
        PersistableHelper.Load((IPersistable)K, storage, "K");
        PersistableHelper.Load((IPersistable)D, storage, "D");
    }

    public override void Save(SettingsStorage storage)
    {
        base.Save(storage);
        storage.SetValue<SettingsStorage>("K", PersistableHelper.Save((IPersistable)K));
        storage.SetValue<SettingsStorage>("D", PersistableHelper.Save((IPersistable)D));
    }
}

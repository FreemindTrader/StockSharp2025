                    ...
                    while ( !IsDisposed && !token.IsCancellationRequested )
                    {
                        ...
                        while ( loadDateInUtc <= stopDateInUtc && !_isChanged && !token.IsCancellationRequested )
                        {
                            if ( !checkDates || boards.Any( b => b.IsTradeDate( currentTime, true ) ) )
                            {
                                ...
                                if ( noData )
                                    EnqueueMessages( startDateTime, stopDateTime, loadDateInUtc, startTime, token, GetSimpleTimeLine( boards, currentTime, MarketTimeChangedInterval ) );
                                else
                                    EnqueueMessages( startDateTime, stopDateTime, loadDateInUtc, startTime, token, messages );
                            }
...
namespace StockSharp.Algo
{
    ...
    public class fxHistoryMessageAdapter : MessageAdapter
	{
		...
		private void ProcessMarketDataMessage( MarketDataMessage message )
		{
			...
			{
				error = new InvalidOperationException( LocalizedStrings.NotSupportedDataForSecurity.Put( dataType ) );
			}

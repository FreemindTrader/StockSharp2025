        ...
        private static readonly Dictionary< TimeSpan, string > dictionary_0 = new Dictionary< TimeSpan, string >( )
    {
      ...
      },
      {
        ...
        public static IEnumerable< TimeSpan > TimeFrames
        {
            get
            {
                return ( IEnumerable< TimeSpan > )QuandlHistorySource.timeSpan_0;
            }
        }
          ...
          object arg )
        {
            ...
            return Path.Combine( this.DumpFolder, string.Format( "{0}_{1}_{2}_{3}_{4:yyyy_MM_dd_HH_mm}_{5:yyyy_MM_dd_HH_mm}.xml", ( object )dataType.Name.ToLowerInvariant( ), ( object )TraderHelper.CandleArgToFolderName( arg ), ( object )this.method_0( security ), ( object )security.Id.SecurityIdToFolderName( ), ( object )from, ( object )to ) );
        }
          ...
          DateTime endDate )
        {
            ...
            if( str == null )
            {
                throw new ArgumentOutOfRangeException( nameof( timeFrame ), ( object )class105.timeSpan_0, LocalizedStrings.Str1219 );
            }

            string path = this.CanDump ? this.GetDumpFile( security, beginDate, endDate, typeof( TimeFrameCandleMessage ), ( object )class105.timeSpan_0 ) : ( string )null;
            ...
            {
                SecurityId securityId = security.Id.ToSecurityId( ( SecurityIdGenerator )null );
                ...
                try
                {
                    using( WebResponse response = webRequest.GetResponse( ) )
                    {
                        using( Stream responseStream = response.GetResponseStream( ) )
                        {
                            ...
                            responseStream.CopyTo( ( Stream )class105.memoryStream_0 );
                            if( path != null )
                            {
            ...
            return ( IEnumerable< TimeFrameCandleMessage > )CultureInfo.InvariantCulture.DoInCulture< List< TimeFrameCandleMessage > >( new Func< List< TimeFrameCandleMessage > >( class105.method_0 ) );
        }
                                ...
                                if( criteria.Code.IsEmpty( ) || str2.ContainsIgnoreCase( criteria.Code ) )
                                {
                                    ...
                                    if( securityStorage.LookupById( id ) == null )
                                    {
                                        StockSharp.BusinessEntities.Security security = new StockSharp.BusinessEntities.Security( )
                                        {
                                            ...
                                            Board = this.ExchangeInfoProvider.GetOrCreateBoard( str1, ( Func< string, ExchangeBoard > )null )
                                        };
        ...
        private sealed class Class105
        {
            ...
            {
                XElement root = XDocument.Load( ( Stream )this.memoryStream_0 ).Root;
                ...
                foreach( XContainer xpathSelectElement in root.XPathSelectElements( "//datum[@type='array']" ) )
                {
                    ...
                    DateTimeOffset dto = array[ index ].Value.ToDateTime( "yyyy-MM-dd", ( CultureInfo )null ).ApplyTimeZone( TimeHelper.Est );

...
namespace StockSharp.Algo.History.Forex
{
    public class OandaHistorySource : BaseHistorySource, ISecurityDownloader
    {
        ...
        {
            return "{0}-{1}-{2}".Put( ( object ) dateTime_0.Year.ToString( "0000" ), ( object ) dateTime_0.Month.ToString( "#0" ), ( object ) dateTime_0.Day.ToString( "#0" ) );
        }
      ...
      DateTime to )
        {
            ...
            {
                throw new ArgumentException( LocalizedStrings.Str2094Params.Put( ( object )security.Id ), nameof( security ) );
            }
            ...
            url.QueryString.Append( "base_currency_0", ( object ) strArray1[ 0 ] ).Append( "quote_currency", ( object ) strArray1[ 1 ] ).Append( "start_date", ( object ) OandaHistorySource.smethod_0( from ) ).Append( "end_date", ( object ) OandaHistorySource.smethod_0( to ) );
            using( HttpRequest httpRequest = new HttpRequest( ) )
            {
                httpRequest.UserAgent = Http.ChromeUserAgent( );
                httpRequest.Referer = "http://www.oanda.com/lang/ru/currency/historical-rates/";
                string str = Encoding.UTF8.GetString( httpRequest.Get( ( Uri ) url, ( RequestParams ) null ).ToBytes( ) );
            ...
            class58_1.securityId_0 = security.ToSecurityId( ( SecurityIdGenerator ) null );
            return ( IEnumerable< Level1ChangeMessage > ) CultureInfo.InvariantCulture.DoInCulture< List< Level1ChangeMessage > >( new Func< List< Level1ChangeMessage > >( class58_1.method_0 ) );
        }
      ...
      Func< bool > isCancelled )
        {
            ...
            foreach( HtmlNode selectNode in ( IEnumerable< HtmlNode > ) new HtmlWeb( )
      {
                    ...
                    if( !( lowerInvariant == "indices_block" ) )
                    {
                        if( !( lowerInvariant == "commodities_block" ) )
                        {
                            if( !( lowerInvariant == "bonds_block" ) )
                            {
                                ...
                                {
                                    throw new InvalidOperationException( LocalizedStrings.Str2140Params.Put( ( object )lowerInvariant ) );
                                }
                            }
                            else
                            {

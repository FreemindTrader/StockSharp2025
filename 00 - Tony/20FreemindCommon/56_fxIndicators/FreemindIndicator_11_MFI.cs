...
namespace fx.Indicators
{
    ...
    {
        private double _latestMfi = 0.0;
        protected Task BasicMfiTask( bool fullRecalculation, DataBarUpdateType? updateType, int curIterationBarcount, ref PooledList<Task> tasksList )
        {
            ...
            //ThreadHelper.UpdateThreadName( "BasicMfiTask" );            

            Task first = new Task(() => CalculateMfi( fullRecalculation, updateType, curIterationBarcount ), IndicatorExitToken);
            Task second = first.ContinueWith(
                                                antecedent =>
                                                {
                                                    if (antecedent.Status == TaskStatus.Faulted)
                                                    {
                                                        this.AddErrorLog("CalculateOscillator() caused Exception. Stack = " + antecedent.Exception.InnerException.StackTrace);
                                                    }
                                                    else
                                                    {
                                                        GetMfiOverBoughtOverSold( fullRecalculation, updateType, curIterationBarcount );
                                                    }
        ...
        protected void CalculateMfi( bool fullRecalculation, DataBarUpdateType? updateType, int curIterationBarcount )
        {
            int resultSetLength   = IndicatorResult["MFI"].Count;

            if ( resultSetLength == 0 )
            {
                ProcessMfiNewIndicatorBuffer( curIterationBarcount );
            }
            else
            {
                ProcessMfiExistingBuffer( updateType, curIterationBarcount, resultSetLength );
            }
        }

        private void ProcessMfiNewIndicatorBuffer( int barB4Calculation )
        {
            int startIndex     = 0;
            ...
            if( outNBElement > 0 )
            {
                ...
                {
                    IndicatorResult.AddSetValues( "MFI", outBeginIdx, outNBElement, true, myMfi );
                }

                _latestMfi = myMfi [ outNBElement - 1 ];
                ...
                _fxTradingEventsBindingList.AddMfiValue( Bars.Period.Value, barB4Calculation, ( int ) _latestMfi, barB4Calculation );
            }
        }

        private void ProcessMfiExistingBuffer( DataBarUpdateType? updateType, int barB4Calculation, int resultSetLength )
        {
        ...
        protected void GetMfiOverBoughtOverSold( bool fullRecalculation, DataBarUpdateType? updateType, int curIterationBarcount )
        {
            //    if ( _noCalculationAllowed && ( updateType != DataBarUpdateType.Reloaded ) )
            //    {
            //        return;

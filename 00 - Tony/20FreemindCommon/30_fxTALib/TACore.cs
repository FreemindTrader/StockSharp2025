...
namespace fx.TALib
{
    public static partial class Core
    {
        ...
        public static RetCode SetUnstablePeriod( FuncUnstId id, long unstablePeriod )
        {
            ...
            {
#if ( NETCOREAPP2_0 || NETCOREAPP2_1 || NETCOREAPP3_0 || NETCOREAPP3_1 )
        ...
        private static RetCode TA_INT_EMA( IHistoricBarsRepo bars, int startIdx, int endIdx, double[ ] outReal, out int outBegIdx,
            out int outNbElement, int optInTimePeriod, double optInK1 )
        {
            ...
            {
                prevMA = ( bars[ today++ ].Close - prevMA ) * optInK1 + prevMA;
            }
        ...
        private static RetCode TA_INT_SMA( IHistoricBarsRepo bars, int startIdx, int endIdx, double[ ] outReal, out int outBegIdx,
            out int outNbElement, int optInTimePeriod )
        {
            ...
            do
            {
                ...
                periodTotal -= bars[ trailingIdx++ ].Close;
        ...
        private static void TA_INT_StdDevUsingPrecalcMA( IHistoricBarsRepo bars, double[ ] inMovAvg, int inMovAvgBegIdx, int inMovAvgNbElement,
            double[ ] outReal, int optInTimePeriod )
        {
            int startSum = inMovAvgBegIdx + 1 - optInTimePeriod;
            int endSum = inMovAvgBegIdx;
            ...
            for ( int outIdx = startSum; outIdx < endSum; outIdx++ )
            {
                double tempReal = bars[outIdx].Close;
                tempReal *= tempReal;
                periodTotal2 += tempReal;
            }
            ...
            for ( var outIdx = 0; outIdx < inMovAvgNbElement; outIdx++, startSum++, endSum++ )
            {
                double tempReal = bars[endSum].Close;
                tempReal *= tempReal;
                periodTotal2 += tempReal;
                double meanValue2 = periodTotal2 / optInTimePeriod;

                tempReal = bars[ startSum ].Close;
                tempReal *= tempReal;
                periodTotal2 -= tempReal;
        ...
        private static RetCode TA_INT_VAR( IHistoricBarsRepo bars, int startIdx, int endIdx, double[ ] outReal, out int outBegIdx,
            out int outNbElement, int optInTimePeriod )
        {
            outBegIdx = outNbElement = 0;
            ...
            do
            {
                ...
                double meanValue1 = periodTotal1 / optInTimePeriod;
                double meanValue2 = periodTotal2 / optInTimePeriod;
                tempReal = bars[ trailingIdx++ ].Close;
                periodTotal1 -= tempReal;
                tempReal *= tempReal;
                periodTotal2 -= tempReal;
                outReal[ outIdx++ ] = meanValue2 - meanValue1 * meanValue1;
        ...
        public enum CandleSettingType : byte
        {
            BodyLong,
            BodyVeryLong,
            BodyShort,

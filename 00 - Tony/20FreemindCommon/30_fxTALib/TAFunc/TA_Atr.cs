...
namespace fx.TALib
{
    ...
    {
        public static RetCode Atr( IHistoricBarsRepo bars, int startIdx, int endIdx, double[ ] outReal,
            out int outBegIdx, out int outNbElement, int optInTimePeriod = 14 )
        {
            ...
            {
                return TRange( bars, startIdx, endIdx, outReal, out outBegIdx, out outNbElement );
            }

            var prevATRTemp = new double[1];

            var tempBuffer = new double[lookbackTotal + (endIdx - startIdx) + 1];
            RetCode retCode = TRange(bars, startIdx - lookbackTotal + 1, endIdx, tempBuffer, out _, out _);

...
namespace fx.TALib
{
    ...
    {
        public static RetCode MinusDM( IHistoricBarsRepo bars, int startIdx, int endIdx, double[ ] outReal, out int outBegIdx,
            out int outNbElement, int optInTimePeriod = 14 )
        {
            ...
            if ( optInTimePeriod == 1 )
            {
                outBegIdx = startIdx;
                today = startIdx - 1;
                prevHigh = bars[ today ].High;
                prevLow = bars[ today ].Low;
                while ( today < endIdx )
                {
                    today++;
                    double tempReal = bars[today].High;
                    diffP = tempReal - prevHigh;
                    prevHigh = tempReal;
                    tempReal = bars[ today ].Low;

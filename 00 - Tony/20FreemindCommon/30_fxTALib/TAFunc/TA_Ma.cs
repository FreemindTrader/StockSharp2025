...
namespace fx.TALib
{
    ...
    {
        public static RetCode Ma( IHistoricBarsRepo bars, int startIdx, int endIdx, double[ ] outReal, out int outBegIdx, out int outNbElement, MAType optInMAType = MAType.Sma, int optInTimePeriod = 30 )
        {
            ...
            if ( bars == null || outReal == null || optInTimePeriod < 1 || optInTimePeriod > 100000 )
            {
            ...
            if ( optInTimePeriod == 1 )
            {
                ...
                {
                    outReal[ outIdx ] = bars[ todayIdx ].Close;
                }
            ...
            {
                case MAType.Sma:
                    return Sma( bars, startIdx, endIdx, outReal, out outBegIdx, out outNbElement, optInTimePeriod );
                case MAType.Ema:
                    return Ema( bars, startIdx, endIdx, outReal, out outBegIdx, out outNbElement, optInTimePeriod );
                case MAType.Wma:
                    return Wma( bars, startIdx, endIdx, outReal, out outBegIdx, out outNbElement, optInTimePeriod );
                case MAType.Dema:
                    return Dema( bars, startIdx, endIdx, outReal, out outBegIdx, out outNbElement, optInTimePeriod );
                case MAType.Tema:
                    return Tema( bars, startIdx, endIdx, outReal, out outBegIdx, out outNbElement, optInTimePeriod );
                case MAType.Trima:
                    return Trima( bars, startIdx, endIdx, outReal, out outBegIdx, out outNbElement, optInTimePeriod );
                case MAType.Kama:
                    return Kama( bars, startIdx, endIdx, outReal, out outBegIdx, out outNbElement, optInTimePeriod );
                case MAType.Mama:
                    var dummyBuffer = new double[endIdx - startIdx + 1];
                    return Mama( bars, startIdx, endIdx, outReal, dummyBuffer, out outBegIdx, out outNbElement );
                case MAType.T3:
                    return T3( bars, startIdx, endIdx, outReal, out outBegIdx, out outNbElement, optInTimePeriod );
                default:
                    return RetCode.BadParam;
            }
        }
        public static RetCode Ma( double[ ] inReal, int startIdx, int endIdx, double[ ] outReal, out int outBegIdx, out int outNbElement, MAType optInMAType = MAType.Sma, int optInTimePeriod = 30 )
        {
            outBegIdx = outNbElement = 0;

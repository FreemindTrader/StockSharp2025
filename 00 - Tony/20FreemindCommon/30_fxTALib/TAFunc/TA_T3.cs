...
namespace fx.TALib
{
    ...
    {
        public static RetCode T3( IHistoricBarsRepo bars, int startIdx, int endIdx, double[ ] outReal, out int outBegIdx, out int outNbElement,
            int optInTimePeriod = 5, double optInVFactor = 0.7 )
        {
            ...
            if ( bars == null || outReal == null || optInTimePeriod < 2 || optInTimePeriod > 100000 || optInVFactor < 0.0 ||
                optInVFactor > 1.0 )
            {
            ...
            for ( int i = optInTimePeriod - 1; i > 0; i-- )
            {
                e1 = k * bars[ today++ ].Close + oneMinusK * e1;
                tempReal += e1;
            }

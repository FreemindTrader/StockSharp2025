...
namespace fx.TALib
{
    ...
    {
        public static RetCode TRange( IHistoricBarsRepo bars, int startIdx, int endIdx, double[ ] outReal,
            out int outBegIdx, out int outNbElement )
        {
            ...
            while ( today <= endIdx )
            {
                double tempLT = bars[today].Low;
                double tempHT = bars[today].High;
                double tempCY = bars[today - 1].Close;

...
namespace fx.Charting
{
    public partial class ChartExViewModel : ViewModelBase, IChart, IPersistable, IThemeableChart
    {
        ...
        {                        
            ...
            if ( chartUi == null )
            {
                MessageBoxResult canCloseDocument = MessageBoxService.Show( messageBoxText: LocalizedStrings.Str2004, caption: "Add Indicator", button: MessageBoxButton.OK );
            }
            else
            {
        ...
        public bool YAxisIsAutoRange
        {
            ...
            set
            {
                ...
                foreach ( ChartArea area in ChartAreas )
                {
                    area.YAxises.ForEach( ax => ax.AutoRange = value );
                }
        ...
        private sealed class ChartAreasList : BaseList<ChartArea>, IPersistable
        {
            ...
            public void UpdateQuotes( DateTime offerTime, double bid, double ask )
            {
                foreach ( ChartArea chartArea in ( CollectionBase<ChartArea> )this )
                {
                    chartArea.ChartSurfaceViewModel.UpdateQuote( offerTime,  bid, ask );
                }
            }

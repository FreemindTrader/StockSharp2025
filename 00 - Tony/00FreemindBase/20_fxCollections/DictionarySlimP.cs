...
namespace fx.Collections
{
    ...
    public class DictionarySlimP<TKey, TValue> : IReadOnlyCollection<KeyValuePair<TKey, TValue>> where TKey : IEquatable<TKey>
    {
        ...
        public DictionarySlimP(int capacity)
        {
            if (capacity < 0)
        ...
        private Entry[] Resize()
        {
            ...
            while (count-- > 0)
            {
                int bucketIndex = entries[count].key.GetHashCode() & (buckets.Length - 1);
                entries[count].next = buckets[bucketIndex] - 1;
        ...
        private void ReturnArrays()
        {
            if (_entries.Length > 1)
            {
                try
                {
                    s_entryPool.Return(_entries, false );
                }
        ...
        {
            private readonly DictionarySlimP<TKey, TValue> _dictionary;
            private int _index;
            private int _count;
            private KeyValuePair<TKey, TValue> _current;

            internal Enumerator(DictionarySlimP<TKey, TValue> dictionary)
            {

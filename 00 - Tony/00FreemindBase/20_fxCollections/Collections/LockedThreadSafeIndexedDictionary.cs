...
namespace fx.Collections
{
    ...
    /// </summary>
    public sealed class LockedThreadSafeIndexedDictionary< TKey, TValue > : IDisposableDictionary< TKey, TValue > where TKey : notnull 
    {
        #region _owner - Single private field

        private ThreadSafeIndexedDictionary< TKey, TValue > _owner;

        #endregion

        #region Constructor        

        internal LockedThreadSafeIndexedDictionary( ThreadSafeIndexedDictionary< TKey, TValue > owner )
        {
            _owner = owner;
        }
        ...
        private ThreadSafeIndexedDictionary< TKey, TValue > _GetOwner( )
        {
            var owner = _owner;
        ...
        public bool TryGetValueAndRemove( TKey key, out TValue value )
        {
            ...
            if( node != null )
            {
                owner._RemoveNode( key, bucketIndex, previousNode, node );

...
namespace fx.Collections
{
    ...
    /// </summary>
    public sealed class ThreadSafeIndexedDictionary< TKey, TValue > : IThreadSafeDictionary< TKey, TValue >, IEnumerable< KeyValuePair< TKey, TValue > > where TKey : notnull 
    {
        ...
        /// </summary>
        public ThreadSafeIndexedDictionary( ) : this( 31 )
        {
        }
        ...
        /// </summary>
        public ThreadSafeIndexedDictionary( int capacity ) : this( capacity, null!, null! )
        {
        }
        ...
        /// </summary>
        public ThreadSafeIndexedDictionary( IReaderWriterLockSlim readerWriterLock ) : this( 31, readerWriterLock, null! )
        {
        }
        ...
        /// </summary>
        public ThreadSafeIndexedDictionary( IEqualityComparer< TKey > comparer ) : this( 31, null!, comparer )
        {
        }
        ...
        public ThreadSafeIndexedDictionary( int capacity, IReaderWriterLockSlim readerWriterLock, IEqualityComparer< TKey > comparer )
        {
            if( comparer == null )
            {
        ...
        public TValue LastValue
        {
            get
            {
                if( _indexCount > 0 )
                {
                    return this[ _indexer[ _indexCount - 1 ] ];
                }
        ...
        internal void _Resize( )
        {
            ...
            try
            {
                _SetCapacitymSizeMustBeAPrime( newCapacity );
            }
        ...
        internal void _GetNodeAndPreviousNode(
                                                    int bucketIndex,
                                                    int hashCode,
                                                    TKey key,
                                                    out _ThreadSafeDictionaryNode< TKey, TValue > resultNode,
                                                    out _ThreadSafeDictionaryNode< TKey, TValue > resultPrviousNode )
        {
        ...
        #region _SetCapacitymSizeMustBeAPrime

        private void _SetCapacitymSizeMustBeAPrime( int newCapacity )
        {
            var oldBuckets = _buckets;
            var newBuckets = new _ThreadSafeDictionaryNode< TKey, TValue >[ newCapacity ];
            TKey[ ] newIndexer = new TKey[ newCapacity * 2 ];
            ...
            Array.Copy( _indexer, newIndexer, _indexCount );
            _indexer = newIndexer;
        }
        ...
        internal void _RemoveNode( TKey key, int bucketIndex, _ThreadSafeDictionaryNode< TKey, TValue > oldNode, _ThreadSafeDictionaryNode< TKey, TValue > node )
        {
            if( oldNode != null )
            {
            ...
            // Tony: Removing node is kinda dumbass for indexed dictionary as we have to copy the old keys to the new array.

            TKey[ ] newIndexer = new TKey[ _indexer.Length ];

            int newIndexerCount = 0;

            for( int i = 0; i < _indexCount; i++ )
            {
                if( _indexer[ i ].Equals( key ) )
                {
                    continue;
                }

                newIndexer[ newIndexerCount ] = _indexer[ i ];
                newIndexerCount++;
            }
        ...
        public bool TryGetValueAndRemove( TKey key, out TValue value )
        {
            ...
            try
            {
                ...
                if( node != null )
                {
                    _RemoveNode( key, bucketIndex, previousNode, node );
        ...
        internal void _Clear( )
        {
            ...
            Array.Clear( _indexer, 0, _indexer.Length );
        }
        ...
        /// </summary>
        public LockedThreadSafeIndexedDictionary< TKey, TValue > Lock( )
        {
            var result = new LockedThreadSafeIndexedDictionary< TKey, TValue >( this );
            _lock.EnterWriteLock( );
            return result;
        }
        ...
        internal void _RemoveMany( RemoveManyDictionaryDelegate< TKey, TValue > checkDelegate )
        {
            ...
            for( int bucketIndex = 0; bucketIndex < count; bucketIndex++ )
            {
                ...
                while( node != null )
                {
                    ...
                    {
                        _RemoveNode( node._key, bucketIndex, oldNode, node );
                    }
        ...
        internal void _AddMany( IEnumerable< KeyValuePair< TKey, TValue > > pairs, int pairCount )
        {
            ...
            {
                _SetCapacitymSizeMustBeAPrime( _DictionaryHelper._AdaptLength( Math.Max( newCount, Capacity * 2 ) ) );
            }

...
namespace StockSharp.Xaml.GridControl
{
    public class PriceChartDataProvider : Disposable, IDisposable, IPriceChartDataProvider
    {
        ...
        private void OnValueChange( Security security, IEnumerable<KeyValuePair<Level1Fields, object>> ienumerable_0, DateTimeOffset from, DateTimeOffset to )
        {
            using ( IEnumerator<KeyValuePair<Level1Fields, object>> enumerator = ienumerable_0.GetEnumerator() )
            {
                while ( ( ( IEnumerator )enumerator ).MoveNext() )
                {
                    KeyValuePair<Level1Fields, object> current = enumerator.Current;
                    if ( Fields.Contains( current.Key ) && ( _dataCache.SafeAdd( Tuple.Create( security, current.Key ) ) ).Add( ( Decimal )current.Value ) )
                    {
                        lock ( _lock )
                        {
                            if ( !_isBusy )

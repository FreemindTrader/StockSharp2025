...
namespace StockSharp.Hydra.Core
{
    ...
    public class Hydra : BaseLogReceiver
    {
        ...
        public Hydra( HydraServerSettings serverSettings, MarketEmulatorSettings emulatorSettings, AuthorizationProvider authProvider, Func<string, PermissionCredentials> getPermissions )
        {
            ...
            var csvEntityRegistry = new CsvEntityRegistry( Paths.AppDataPath );
            var exchangeInfoProvider = new StorageExchangeInfoProvider( csvEntityRegistry, true );

            ConfigManager.RegisterService<IEntityRegistry>( csvEntityRegistry );
            var service1 = new CsvNativeIdStorage( Paths.SecurityNativeIdDir )                                  { DelayAction = csvEntityRegistry.DelayAction };
            var service2 = new CsvSecurityMappingStorage( Paths.SecurityMappingDir )                            { DelayAction = csvEntityRegistry.DelayAction };
            var service3 = new CsvExtendedInfoStorage( Paths.SecurityExtendedInfo )                             { DelayAction = csvEntityRegistry.DelayAction };
            var service4 = new CsvSecurityMappingStorage( Path.Combine( Paths.AppDataPath, "Server mapping" ) ) { DelayAction = csvEntityRegistry.DelayAction };
            var service5 = new HydraStorage( Path.Combine( Paths.AppDataPath, "Tasks" ) )                       { DelayAction = csvEntityRegistry.DelayAction };
        ...
        public void Initialize()
        {
            ...
            /* ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
             * 
             * Tony 01: Here is where we instantiate the FXCM connector
             * 
             * ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
             */
            var adapters = ServicesRegistry.AdapterProvider.PossibleAdapters.Select( a => a.GetType( ) );

            instance.Init( adapters, new Func<string, Type>( ConvertConnectorTask ) );
        }

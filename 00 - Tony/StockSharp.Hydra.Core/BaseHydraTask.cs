...
namespace StockSharp.Hydra.Core
{
    ...
    public abstract class BaseHydraTask : BaseLogReceiver, IHydraTask, ILogReceiver, ILogSource, IDisposable, ICloneable<IHydraTask>, ICloneable, IPersistable, INotifyPropertyChanged, IScheduledTask
    {
        ...
        public void Start()
        {
            ...
            ( ( Action )( () =>
            {
                try
                {
                    ...
                    while ( State == TaskStates.Starting && second-- > 0 )
                    {
        ...
        //    {
        //        Decimal? step = priceStep;
        //        Decimal num2 = new Decimal();
        //        if ( !( step.GetValueOrDefault() == num2 & step.HasValue ) )
        //        {                                        
        //            Decimal? nullable4 = price.HasValue & priceStep.HasValue ? new Decimal?( price.GetValueOrDefault() % priceStep.GetValueOrDefault() ) : new Decimal?();
            ...
            SafeSave( security, Messages.DataType.PositionChanges, messages, c => c.ServerTime, new Func<PositionChangeMessage, string>[1]
      {
        ...
        {
            foreach ( IGrouping<ExecutionTypes?, ExecutionMessage> grouping in transactions.GroupBy( e => e.ExecutionType ) )
        ...
        public override void Load( SettingsStorage storage )
        {
            ...
            if ( storage.ContainsKey( "WorkingTime" ) )
                WorkingTime.Load( storage.GetValue<SettingsStorage>( "WorkingTime", null ) );
            if ( storage.ContainsKey( "SupportedLevel1Fields" ) )
            {
                _supportedLevel1Fields.Clear();
                _supportedLevel1Fields.AddRange( storage.GetValue<IEnumerable<string>>( "SupportedLevel1Fields", null ).Select( t => t.To<Level1Fields>() ) );
            }

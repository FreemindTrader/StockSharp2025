    ...
    public partial class Chart : UserControl, INotifyPropertyChanged, IComponentConnector, IPersistable, INotifyPropertyChangedEx, IChart, IThemeableChart
    {
        ...
        public bool IsAutoRange
        {
            ...
            set
            {
                ...
                {
                    area.XAxises.ForEach( ax => ax.AutoRange = value );
                }
        ...
        public event Action<IfxChartElement> RemoveElement
        {
            add
            {
        ...
        {
            area.Elements.Added   += new Action<IfxChartElement>( OnNewUIAddedToArea );
            ...
            area.Elements.ForEach( new Action<IfxChartElement>( OnNewUIAddedToArea ) );
        }
        ...
        private void OnAreaRemovedFromChartArea( ChartArea area )
        {
            ...
            area.Elements.ForEach( new Action<IfxChartElement>( OnUIRemovedFromArea ) );
            area.Chart= null;
            area.Dispose( );
        }
        ...
        * ------------------------------------------------------------------------------------------------------------------------------------------- 
        */
        private void OnNewUIAddedToArea( IfxChartElement element )
        {
            if ( !IsInteracted )
            {
                return;
        ...
        private void OnAddIndicatorArea( ChartArea area )
        {
            ...
            if ( CandlestickUI == null )
            {
                new MessageBoxBuilder( ).Error( ).Text( LocalizedStrings.Str2004 ).Owner( this ).Show( );
            }
            else
            {
                ...
                var painter                  = indicatorPicker.SelectedIndicatorType.Painter;
        ...
        private sealed class ChartAreasList : BaseList<ChartArea>, IPersistable
        {
            ...
            public void DrawChartAreas( ChartDrawDataEx drawData )
            {
                ...
                * ------------------------------------------------------------------------------------------------------------------------------------------- 
                */
                foreach ( ChartArea chartArea in ( CollectionBase<ChartArea> )this )
                {
                                  ...
                                  IDictionary<Guid, IIndicator> indicatorSeries )
            {
                ...
                foreach ( SettingsStorage storage in settings.GetValue<IEnumerable<SettingsStorage>>( "Areas", null ) )
                {
                    ...
                    {
                        object candle = candleSeries.TryGetValue( element.Id );
                        if ( candle != null )
                        {
                        ...
                        {
                            IIndicator iindicator = indicatorSeries.TryGetValue( key.Id );
                            if ( candle != null )
                            {

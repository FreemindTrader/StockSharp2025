...
namespace fx.TALib
{
    ...
    {
        public static RetCode Cci( IHistoricBarsRepo bars, int startIdx, int endIdx, double[ ] outReal,
            out int outBegIdx, out int outNbElement, int optInTimePeriod = 14 )
        {
            ...
            while ( i < startIdx )
            {
                circBuffer[ circBufferIdx++ ] = ( bars[ i ].High + bars[ i ].Low + bars[ i ].Close ) / 3.0;
                i++;
            ...
            do
            {
                double lastValue = ( bars[ i ].High + bars[ i ].Low + bars[ i ].Close ) / 3.0;

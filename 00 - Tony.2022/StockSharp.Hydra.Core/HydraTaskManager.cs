...
namespace StockSharp.Hydra.Core
{
    ...
    public class HydraTaskManager : BaseLogReceiver
    {
        ...
        public void Init( IEnumerable<Type> adapters, Func<string, Type> migration )
        {
            ...
            tasks.AddRange( adapters.Select( a => typeof( ConnectorHydraTask<> ).Make( a ) ) );
                    ...
                    else
                    {
                        ...
                        else
                        {
                            try
                            {
                                ...
                                tasks.AddRange( assembly.GetTypes().Where( t =>
                                                                                {
            ...
            AvailableTasks = tasks.ToArray();
            var exceptionContext = new ContinueOnExceptionContext();
            exceptionContext.Error += ex => ex.LogError( null );

            var dictionary = Storage.Load( this );
                ...
                else
                {
                    foreach ( HydraTaskInfo index in ( IEnumerable<HydraTaskInfo> )grouping )
                    {
                        try
                        {
                            /* -------------------------------------------------------------------------------------------------------------------------------------------
                            * 
                            *  Tony: Here we Create an Instance of Connector to download databars
                            * 
                            * ------------------------------------------------------------------------------------------------------------------------------------------- */
                            IHydraTask hydraTask = type1.CreateInstance<IHydraTask>();
                            hydraTask.Parent = this;
                            hydraTask.Init( index.Id );

                            var cachedSynchronizedSet = new CachedSynchronizedSet<HydraTaskSecurity>();
                            cachedSynchronizedSet.AddRange( dictionary[index] );
                            
                            if ( cachedSynchronizedSet.Count == 0 )
                            {
                                HydraTaskSecurity all = CreateAll( hydraTask );
                                cachedSynchronizedSet.Add( all );
                                Storage.Add( index.Id, new HydraTaskSecurity[1] { all } );
                            }

                            hydraTask.Securities = cachedSynchronizedSet.Cache;
                            hydraTask.Load( index.Settings );
                            _taskSecurities.Add( hydraTask, cachedSynchronizedSet );
